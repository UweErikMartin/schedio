...
The **Booking-Calendar** is a read-only computed view of domain `Booking`
records. Booking events are created exclusively by the domain layer. `PUT` and
`DELETE` requests targeting the Booking-Calendar must be rejected with
...
— expands the recurrence rule locally. A recurring series root must therefore
always be synthesised as **free** (`TRANSP: TRANSPARENT`, `SUMMARY: "Free"`),
regardless of whether individual occurrences happen to be booked. Individual
...
## 8. Booking Read Path (Booking-Calendar)
...
`REPORT` (calendar-query) requests on the Booking-Calendar must pre-filter
...
## 9. PROPFIND Privilege Injection
...
## 10. Scheduling Support (Inbox / Outbox)
...
An attempt to modify a read-only resource (e.g. write to the Booking-Calendar,
...
## 18. Supported CalDAV Methods Summary
